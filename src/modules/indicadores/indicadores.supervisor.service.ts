...
export class IndicadoresSupervisorService {
    ...
    async getTable(mes:string, canal:string, nome_supervisor:string):Promise<any[]>{
    ...
    async getQuartilTma(mes: string, canal: string, nome_supervisor: string): Promise<any> {
        if (canal === 'CHAT') {
            const operadores =
            await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_CHAT WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}' ORDER BY tma ASC`)
            return await this.dividirEmQuartis(operadores, 'tma')
        }
        else {
            const operadores =
            await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_VOZ WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}' ORDER BY tma ASC`)
            return await this.dividirEmQuartis(operadores, 'tma')
        }
    }
    async getQuartilCsat(mes: string, canal: string, nome_supervisor: string): Promise<any> {
        if (canal === 'CHAT') {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_CHAT WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY csat ASC`)
            return await this.dividirEmQuartis(operadores, 'csat')
        }
        else {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_VOZ WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY csat ASC`)
            return await this.dividirEmQuartis(operadores, 'csat')
        }
    }
    async getQuartilNotaQualide(mes: string, canal: string, nome_supervisor: string): Promise<any> {
        if (canal === 'CHAT') {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_CHAT WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY nota_qualidade ASC`)
            return await this.dividirEmQuartis(operadores, 'nota_qualidade')
        }
        else {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_VOZ WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY nota_qualidade ASC`)
            return await this.dividirEmQuartis(operadores, 'nota_qualidade')
        }
    }
    async getQuartilNotaVenda(mes: string, canal: string, nome_supervisor: string): Promise<any> {
        if (canal === 'CHAT') {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_CHAT WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY nota_venda ASC`)
            return await this.dividirEmQuartis(operadores, 'nota_venda')
        }
        else {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_VOZ WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY nota_venda ASC`)
            return await this.dividirEmQuartis(operadores, 'nota_venda')
        }
    }
    async getQuartilVenda(mes: string, canal: string, nome_supervisor: string): Promise<any> {
        if (canal === 'CHAT') {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_CHAT WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY qtd_vendas ASC`)
            return await this.dividirEmQuartis(operadores, 'qtd_vendas')
        }
        else {
            const operadores =
                await this.databaseService.query(`SELECT * FROM dbo.MAPA_GESTAO_VOZ WHERE supervisor = '${nome_supervisor}' AND mes = '${mes}'  ORDER BY qtd_vendas ASC`)
            return await this.dividirEmQuartis(operadores, 'qtd_vendas')
        }
    }
